...
void salereport()
{
    ...
    while (fread(&m, sizeof(salesrpt), 1, p))
    {
        gotoxy(3, i);
        SetColor(7);
        printf("%-4d    %-20s   %-30s  %-3d  %-3.2f  %-4.2f  %02d/%02d/%-4d", m.medi_id, m.medir_name, m.cust_name, m.qty, m.rate, m.total, m.sDay, m.sMonth, m.sYear);
        i = i + 2;
    }
    fclose(p);
    i += 2;
...
void purreport()
{
    ...
    while (fread(&m, sizeof(purchaserpt), 1, p))
    {
        gotoxy(3, i);
        SetColor(7);
        printf("%-4d    %-20s   %-30s  %-3d  %-3.2f  %-4.2f  %2d/%2d/%-4d", m.medi_id, m.medi_name, m.supp_name, m.qty, m.rate, m.total, m.sDay, m.sMonth, m.sYear);
        i = i + 2;
    }
    fclose(p);
    i += 2;
